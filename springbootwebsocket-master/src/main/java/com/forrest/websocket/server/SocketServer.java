...
public class SocketServer {
	 ...
	 */
	private final static String SYS_USERNAME = "forrest07";
    ...
    public synchronized static void sendMessage(int[] message,String userName) {
    ...
    public synchronized static void SendReset(String against,int a) {
		// TODO Auto-generated method stub
    	socketServers.forEach(client ->{
			if (against.equals(client.getUserName())) {
				try {
					String msg = "{\"reset\":\""+a+"\"}";
					client.getSession().getBasicRemote().sendText(msg);
	 ...
	 * @param message
	 */
	public synchronized static void sendAll(String username, String message,String filename) {
		//群发，不能发送给服务端自己
		socketServers.stream().filter(cli -> !cli.getUserName(). equals( username))
				.forEach(client -> {
			...
			//	map.put("username", username);
				String msg = "{\"username\":\""+username+"\",\"message\":\""+message+"\",\"filename\":\""+filename+"\"}";
		...
		logger.info("【{}】推送给所有客户端 :{}",username,message);
	}
	...
	public synchronized static void SendChoose(String message,String userName) {
		
		sendMessage(message,userName);
		
	}

	public synchronized static void SendQizi(int[] position, String against) {
		
		sendMessage(position,against);
